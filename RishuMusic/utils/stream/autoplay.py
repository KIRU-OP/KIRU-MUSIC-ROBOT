...
from RishuMusic.platforms.Youtube import YouTubeAPI, youtube_search_multi
...
_THUMB_CACHE = {}          # {video_id: url} — avoids re-checking the same video repeatedly
_THUMB_TIMEOUT = aiohttp.ClientTimeout(total=4)


async def get_thumbnail_direct(video_id: str) -> str:
    """
    Return the best available YouTube thumbnail URL for a video_id.

    Tries maxresdefault -> hqdefault -> mqdefault -> default, in order of
    quality, using lightweight HEAD requests (no image body downloaded)
    with a short timeout so a slow/unreachable YouTube CDN can't stall
    autoplay. Results are cached per video_id since the same song is
    often looked up more than once in a session.
    """
    if not video_id:
        return ""
    ...
    try:
        async with aiohttp.ClientSession(timeout=_THUMB_TIMEOUT) as session:
            for url in urls:
                try:
                    async with session.head(url, allow_redirects=True) as resp:
                        # YouTube serves a small placeholder (grey box, ~1-2KB)
                        # for sizes that don't exist for a given video, instead
                        # of a 404. Content-Length lets us skip those fakes.
                        content_length = int(resp.headers.get("Content-Length", "0") or 0)
                        if resp.status == 200 and content_length > 1000:
                            _THUMB_CACHE[video_id] = url
                            return url
                except Exception:
                    continue
    except Exception:
    ...
    # Nothing verified — fall back to hqdefault, which YouTube generates
    # for virtually every uploaded video.
    fallback = urls[1]
    _THUMB_CACHE[video_id] = fallback
    return fallback
...
async def auto_play_next(
    ...
    try:
        ...
        from RishuMusic.utils.database import is_active_chat as _verify_stream_active
